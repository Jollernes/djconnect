...
import { DenmarkDJMap } from "@/components/wedding/DenmarkDJMap";
...
export function WeddingDJsTestCPage() {
  useDocumentHead({
    title: "[Test C] Wedding DJs · Map of Denmark · DJConnect",
    description: "Design test variant — Denmark map integrated into the listings rail.",
  ...
  return (
    <div className="bg-gradient-to-b from-white via-white to-slate-50">
      <div className="border-b bg-amber-50/70">
        <div className="container py-2 text-xs">
          ...
          <span className="text-amber-800">
            Variant C · Denmark map integrated into the listings rail · Horizontal filters
          </span>
        </div>
      </div>
      <WeddingDJsHero />
      ...
      <section className="container py-6 sm:py-8">
        ...
        {/* Two-column listings layout: DJ grid (main) + sticky map rail (right) */}
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-8">
                    ...
                    {availableWithDummies.map((dj) => (
                      <div
                        ...
                        className={cn(
                          ...
                          highlightId === dj.id
                            ? "-translate-y-1 rounded-2xl ring-4 ring-rose-300 ring-offset-2"
                            : "",
                        )}
                      >
                        <DJCard dj={dj} eventTypeId="wedding" />
                      </div>
                    ))}
                  </div>
                ) : (
          ...
          {/* Map rail (right column on desktop, stacks above grid on mobile) */}
          <aside className="order-1 lg:order-2">
            <div className="lg:sticky lg:top-32">
              <div className="flex items-baseline justify-between gap-2">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-rose-600">
                  DJs on the map
                </h3>
              ...
              <DenmarkDJMap
                djs={availableWithDummies}
                highlightId={highlightId}
                onPinClick={(id) => {
                  setHighlightId(id);
                  const card = document.getElementById(`dj-card-${id}`);
                  if (card) {
                    card.scrollIntoView({ behavior: "smooth", block: "center" });
                    window.setTimeout(() => setHighlightId(null), 2400);
                  }
                }}
                className="mt-2 aspect-[1000/810] w-full"
              />
              <p className="mt-3 text-xs text-muted-foreground">
                Each pin is a verified wedding DJ. Most travel up to 100 km from
                their base — Aarhus DJs cover Aalborg, Copenhagen DJs cover Sjælland.
              </p>
            </div>
          </aside>
        </div>
