  ...
  return (
    <div className="container py-8">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold sm:text-3xl">Find DJs</h1>
          <p className="text-sm text-muted-foreground">
            {loading ? "Søger…" : `${djs.length} verificeret${djs.length === 1 ? "" : "e"} DJ${djs.length === 1 ? "" : "s"} ledig`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:w-64">
            ...
            <Input
              placeholder="DJ-navn, bio eller lokation"
              value={filters.query ?? ""}
          ...
          <Select value={filters.sortBy} onValueChange={(v) => update({ sort: v })}>
            ...
            <SelectContent>
              ...
              <SelectItem value="rating">Bedømmelse</SelectItem>
              <SelectItem value="most_reviewed">Flest anmeldelser</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        <aside className="space-y-6 rounded-xl border bg-card p-5">
          <div className="flex items-center justify-between">
            ...
            {activeCount > 0 && (
              <Button variant="ghost" size="sm" onClick={clearAllFilters}>
                <X className="h-3.5 w-3.5" /> Ryd
              </Button>
            )}
          </div>

          <div>
            <Label htmlFor="city-filter">Lokation</Label>
            <Input
              id="city-filter"
              className="mt-1"
          ...
          <div>
            <Label htmlFor="date-filter">Ledig den</Label>
            <Input
              id="date-filter"
              type="date"
              className="mt-1"
          ...
          <div>
            ...
            >
              <SelectTrigger className="mt-1"><SelectValue placeholder="Alle størrelser" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="__any">Alle størrelser</SelectItem>
                {SETUP_SIZES.map((s) => (
          ...
          <div>
            <Label>Minimumsbedømmelse</Label>
            <Select
              value={String(filters.minRating ?? "")}
              onValueChange={(v) => update({ minRating: v === "__any" ? undefined : v })}
            >
              <SelectTrigger className="mt-1"><SelectValue placeholder="Alle bedømmelser" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="__any">Alle bedømmelser</SelectItem>
                <SelectItem value="4">4★ og op</SelectItem>
                <SelectItem value="4.5">4,5★ og op</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div>
            <Label htmlFor="max-price">Maks. pris (DKK)</Label>
            <Input
              id="max-price"
              type="number"
              className="mt-1"
...
Eventtypen er valgt ovenfor og gælder for alle viste DJs. Tryk "Skift event" for at ændre.
          </p>
        </aside>
        ...
        <div>
          {loading ? (
            ...
            <EmptyState
              ...
              description="Prøv at løsne nogle filtre eller udvide din lokation."
              action={<Button onClick={clearAllFilters}>Ryd filtre</Button>}
            />
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {djs.map((dj) => (
